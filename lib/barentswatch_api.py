...
def fetch_and_store_track(db, access_token, mmsi, msgtimefrom, msgtimeto, config):
    ...
    destination = positions[0].get('destination') or ''
    callsign = positions[0].get('callSign') or ''
    length = positions[0].get('dimensionA', 0) + positions[0].get('dimensionB', 0)
    width = positions[0].get('dimensionC', 0) + positions[0].get('dimensionD', 0)
